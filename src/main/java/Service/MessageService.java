...
public class MessageService {
    ...
    public Optional<Message> getMessageById(int id) {
        ...
        try {
            ...
            if (!message.isPresent()) {
                throw new ExceptionSer("Message not found");
            }
            LOGGER.info("Fetched message: {}", message.orElse(null));
            return message;
        } catch (DaoException e) {
            throw new ExceptionSer(DB_ACCESS_ERROR_MSG, e);
        }
    }
    ...
    public Message createMessage(Message message, Optional<Account> account) {
        ...
        if (!account.isPresent()) {
            throw new ExceptionSer("Account must exist when posting a new message");
        }
    ...
    private void validateMessage(Message message) {
        ...
        if (message.getMessage_text() == null || message.getMessage_text().trim().isEmpty()) {
            throw new ExceptionSer("Message text cannot be null or empty");
        }
        if (message.getMessage_text().length() > 254) {
            throw new ExceptionSer("Message text cannot exceed 254 characters");
        }
    }
    ...
    private void checkAccountPermission(Account account, int postedBy) {
        ...
        if (account.getAccount_id() != postedBy) {
            throw new ExceptionSer("Account not authorized to modify this message");
        }
    }
}